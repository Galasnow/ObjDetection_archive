...
void NanoDetPlus::preprocess(JNIEnv *env, jobject image, ncnn::Mat& in)
{
...
void NanoDetPlus::decode_infer(ncnn::Mat& feats, std::vector<CenterPrior>& center_priors, float threshold, std::vector<std::vector<BoxInfo>>& results, float width_ratio, float height_ratio)
{
...
BoxInfo NanoDetPlus::disPred2Bbox(const float*& dfl_det, int label, float score, int x, int y, int stride, float width_ratio, float height_ratio)
{
    float ct_x = x * stride;
    float ct_y = y * stride;
